...
class _BatchCallback(object):
    ...
    """
        ...
        if exception is None:
            ...
            self.failure += 1
            logging.error('Failed to insert item for user %s: %s',
...
<<<<<<< HEAD
        ...
        try:
            template_values['contact'] = \
                self.mirror_service.contacts().get(id='Python Quick Start'
                    ).execute()
        except errors.HttpError:
            logging.info('Unable to find Python Quick Start contact.')

        timeline_items = \
            self.mirror_service.timeline().list(maxResults=3).execute()
        template_values['timelineItems'] = timeline_items.get('items',
                [])

        subscriptions = \
            self.mirror_service.subscriptions().list().execute()
    ...
    # Dict of operations to easily map keys to methods.
        ...
        operations = {
            ...
            'insertVideo': self._insert_video,
...
=======
    try:
      ...
      logging.info('Unable to  Python Quick Start contact.')
  ...
  def post(self):
    ...
    operations = {
        ...
        'deleteSubscription': self._delete_subscription,
        'insertItem': self._insert_item,
        'insertVideo1': self._insert_video1,
        'insertVideo2': self._insert_video2,
        'insertAudio':self._insert_audio,
        'insertItemWithAction': self._insert_item_with_action,
        'insertItemAllUsers': self._insert_item_all_users,
        'insertContact': self._insert_contact,
        'deleteContact': self._delete_contact,
...
>>>>>>> test
    ...
    # self.userid is initialized in util.auth_required.
        ...
        body = {'collection': self.request.get('collection', 'timeline'
...
>>>>>>> test
  ...
  def _insert_video1(self):
        """Insert a timeline item."""
        logging.info('Inserting timeline item')
        body = {
            'notification': {'level': 'DEFAULT'},
            'text': 'Hiking Trail Part 1',
            'menuItems': [{'action': 'DELETE'}],
            'bundleId':'thetrailremap2013'
        }
        media_link = util.get_full_url(self, '/static/videos/1.mp4')
        if media_link:
          if media_link.startswith('/'):
            media_link = util.get_full_url(self, media_link)
          resp = urlfetch.fetch(media_link, deadline=60)
  ...
  def _insert_video2(self):
        """Insert a timeline item."""
        logging.info('Inserting timeline item')
        body = {
            'notification': {'level': 'DEFAULT'},
            'text': 'Hiking Trail Part 2',
            'menuItems': [{'action': 'DELETE'}],
            'bundleId':'thetrailremap2013'
        }
        media_link = util.get_full_url(self, '/static/videos/2.mp4')
        if media_link:
          if media_link.startswith('/'):
            media_link = util.get_full_url(self, media_link)
          resp = urlfetch.fetch(media_link, deadline=60)
  ...
  def _insert_audio(self):
        """Insert a timeline item."""
        logging.info('Inserting timeline item')
        txt="something"
        body = {
            'text':txt,
            'speakableText':'You love the music?',
            'notification': {'level': 'DEFAULT'},
            'menuItems': [{'action': 'DELETE'},{'action':'READ_ALOUD'},{'action':'PLAY_VIDEO'}],
            'bundleId':'thetrailremap2013'
        }
    ...
    def _insert_video(self):
        ...
        logging.info('Inserting timeline item')
        body = {'notification': {'level': 'DEFAULT'},
                'text': 'You like this cat?',
                'menuItems': [{'action': 'DELETE'}]}
        media_link = util.get_full_url(self, '/static/videos/test.mp4')
        if media_link:
            if media_link.startswith('/'):
                media_link = util.get_full_url(self, media_link)
            resp = urlfetch.fetch(media_link, deadline=2000)
            media = MediaIoBaseUpload(io.BytesIO(resp.content),
                    mimetype='video/vnd.google-glass.stream-url', resumable=True)
        else:
            media = None
    ...
    def _insert_item_with_action(self):
        ...
        logging.info('Inserting timeline item')
        body = {
            'creator': {'displayName': 'Python Starter Project',
    ...
    def _insert_item_all_users(self):
        ...
        for user in users:
            creds = StorageByKeyName(Credentials, user.key().name(),
                    'credentials').get()
    ...
    # self.mirror_service is initialized in util.auth_required.
        ...
        self.mirror_service.contacts().delete(id=self.request.get('id'
                )).execute()
    ...
    # self.mirror_service is initialized in util.auth_required.
        ...
        self.mirror_service.timeline().delete(id=self.request.get('itemId'
                )).execute()
        return 'A timeline item has been deleted.'


MAIN_ROUTES = [('/', MainHandler)]
